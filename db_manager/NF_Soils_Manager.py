...
def initialize(con, show=False):
    ...
    # logger metadata
    add_logger(logger_model=logger_model, site=site, rep=7, logger_sn=logger_sn, shortname='NF_Soils', comment='soils logger', con=con, show=show)
    ...
    hydra_cols = {}
    hydra_units = {}
    for height, trueheight in zip(range(1, 6), [-0.05, -0.1, -0.15, -0.3, -0.5]):
        for pit, name in zip(range(1, 3), ['A', 'B']):
            ...
            add_instrument(shortname='hydra', site=site, height=trueheight, instr_model='Hydraprobe', rep=pit, instr_sn=False,  comment=f'Pit {name} hydraprobe: SWC, TS, electrical properties', columns=hydra_cols, units=hydra_units, logger_sn=logger_sn,  con=con, show=show)
...
def load_cs30min(con, fns):
    ...
    rx = {f'cs616_{height}':re.compile(f'^WC_C{height}cm.*|^tau616_.*({i+1})') for i, height in enumerate(['5', '10', '15', '30', '50'])}
    rx.update({'status':re.compile('T_CR3000.*|V_CR3000.*')})

    heights = ['neg5cm', 'neg10cm', 'neg15cm', 'neg30cm', 'neg50cm']
    table_names = [f'tdr_NF_{height}_3' for height in heights] + ['status_NF_0cm_7']
...
def load_gsts30min(con, fns):
    ...
    rx.update({f'gs_B{height}':re.compile(f'^Gs_B{height}.*') for height in ['5cm', '10m', '15m']})
    rx.update({f'gs_C{height}':re.compile(f'^Gs_C{height}.*') for height in ['5cm']})
    rx.update({f'ts_C{height}':re.compile(f'^Ts_C{height}.*') for height in ['5cm', '10cm', '15cm', '30cm', '50cm']})
    
    heights = ['neg5cm', 'neg10cm', 'neg15cm', 'neg30cm', 'neg50cm']
    table_names = ([f'soilhf_NF_{height}_1' for height in heights[:3]]
                   + [f'soilhf_NF_{height}_2' for height in heights[:3]]
                   + [f'soilhf_NF_{height}_3' for height in [heights[0]]]
                   + [f'tsoil_NF_{height}_3' for height in heights])
    table_names = {instr:name for instr, name in zip(rx, table_names)}

    # renaming/standardization instructions
    renaming_dict = {"TIMESTAMP":'timestamp',
                    "Gs_A5cm_Avg": 'g_1_1_1-avg',
                    "Gs_A10m_Avg": 'g_1_2_1-avg',
                    "Gs_A15m_Avg": 'g_1_3_1-avg',
                    "Gs_B5cm_Avg": 'g_2_1_1-avg',
                    "Gs_B10m_Avg": 'g_2_2_1-avg',
                    "Gs_B15m_Avg": 'g_2_3_1-avg',
                    "Gs_C5cm_Avg": 'g_3_1_1-avg',
                    ...
                    "Gs_A5cm_Std": 'g_1_1_1-std',
                    "Gs_A10m_Std": 'g_1_2_1-std',
                    "Gs_A15m_Std": 'g_1_3_1-std',
                    "Gs_B5cm_Std": 'g_2_1_1-std',
                    "Gs_B10m_Std": 'g_2_2_1-std',
                    "Gs_B15m_Std": 'g_2_3_1-std',
                    "Gs_C5cm_Std": 'g_3_1_1-std',
